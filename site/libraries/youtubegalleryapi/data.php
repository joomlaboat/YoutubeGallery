...
';
			...
			elseif($vsn=='vimeouservideos')
			{
				...
				$newlist=YGAPI_VideoSource_VimeoUserVideos::getVideoIDList($theLink, $specialparams, $playlistid,$datalink,$active_key, $youtube_data_api_key);
			}
			elseif($vsn=='vimeochannel')
			{
				require_once('providers'.DIRECTORY_SEPARATOR.'vimeochannel.php');
				$newlist=YGAPI_VideoSource_VimeoChannel::getVideoIDList($theLink, $specialparams, $playlistid,$datalink,$active_key, $youtube_data_api_key);
			}
			elseif($vsn=='vimeoalbum')
			{
				require_once('providers'.DIRECTORY_SEPARATOR.'vimeoalbum.php');
				$newlist=YGAPI_VideoSource_VimeoAlbum::getVideoIDList($theLink, $specialparams, $playlistid,$datalink,$active_key, $youtube_data_api_key);
			}
			elseif($vsn=='dailymotionplaylist')
			{
				require_once('providers'.DIRECTORY_SEPARATOR.'dailymotionplaylist.php');
				$newlist=YGAPI_VideoSource_DailymotionPlaylist::getVideoIDList($theLink, $specialparams, $playlistid,$datalink,$active_key, $youtube_data_api_key);
			}
			...
			switch($vsn)
			{
				...
				case 'youtube' :
					...
					if($videoid!='')
					{
						...
						if($key == '')
						{
							JFactory::getApplication()->enqueueMessage('Youtube Data API key is required.', 'error');
							return $videoitem;
						}
	...
	protected static function getVideoData($link,&$blankArray)
	{
		...
		try
		{
			$htmlcode=YouTubeGalleryAPIMisc::getURLData($link);
