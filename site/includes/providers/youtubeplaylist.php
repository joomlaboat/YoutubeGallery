...
class VideoSource_YoutubePlaylist
{
	...
	public static function getPlaylistVideos($playlistid,&$datalink,$api_key,$optionalparameters)
	{
		...
		while($videos_found<$count)
		{
			...
			if($htmlcode=='')
			{
				JFactory::getApplication()->enqueueMessage('Could not connect to https://www.googleapis.com/youtube/v3', 'Error');

				return $videolist;//array();
			}

			$j=json_decode($htmlcode);
			if(!$j)
			{
				JFactory::getApplication()->enqueueMessage('Connection Error. Unexpected response: '.$htmlcode, 'Error');
			...
			if(isset($j->error))
			{
				...
				{
					$e=$j->error->errors[0];
					JFactory::getApplication()->enqueueMessage($e->message, 'Error');
