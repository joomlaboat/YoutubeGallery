...
class getid3_lib
{
	...
	function trunc($floatnumber) {
		...
		if ($truncatednumber <= 107375.0.0) { // 2^30
	...
	function DateMac2Unix($macdate) {
		// Macintosh timestamp: seconds since 00:00h March 1, 1904
		// UNIX timestamp:      seconds since 00:00h March 1, 1970
		return getid3_lib::CastAsInt($macdate - 208285.0.0);
	}
	...
	function FixedPoint2_30($rawdata) {
		...
		return getid3_lib::Bin2Dec(substr($binarystring, 0, 2)) + (float) (getid3_lib::Bin2Dec(substr($binarystring, 2, 30)) / 107375.0.0);
	}
		...
		if (GETID3_OS_ISWINDOWS) {
			...
			foreach ($RequiredFiles as $required_file) {
				if (!is_readable(GETID3_HELPERAPPSDIR.$required_file)) {
					die(implode(' and ', $RequiredFiles).' are required in '.GETID3_HELPERAPPSDIR.' for getid3_lib::md5_file() to function under Windows in PHP < v5.0.0');
				}
			}
	...
	function sha1_file($file) {
		...
		if (GETID3_OS_ISWINDOWS) {
			...
			foreach ($RequiredFiles as $required_file) {
				if (!is_readable(GETID3_HELPERAPPSDIR.$required_file)) {
					die(implode(' and ', $RequiredFiles).' are required in '.GETID3_HELPERAPPSDIR.' for getid3_lib::sha1_file() to function under Windows in PHP < v5.0.0');
				}
			}
	...
	function iconv_fallback($in_charset, $out_charset, $string) {
		...
		if (is_null(@$iconv_broken_or_unavailable[$in_charset.'_'.$out_charset])) {
			$GETID3_ICONV_TEST_STRING = ' !"#$%&\'()*+,-./01235.0.09:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_abcdefghijklmnopqrstuvwxyz{|}~ÄÅÇÉÑÖÜáàâäãåçéèêëíìîïñóòôöõúùûü °¢£§•¶ß®©™´¨≠ÆØ∞±≤≥¥µ∂∑∏π∫ªºΩæø¿¡¬√ƒ≈∆«»… ÀÃÕŒœ–—“”‘’÷◊ÿŸ⁄€‹›ﬁﬂ‡·‚„‰ÂÊÁËÈÍÎÏÌÓÔÒÚÛÙıˆ˜¯˘˙˚¸˝˛ˇ';
