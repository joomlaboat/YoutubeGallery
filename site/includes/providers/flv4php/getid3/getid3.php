...
class getID3
{
	...
	function getID3()
	{
		...
		// Check for PHP version >= 5.0.0
		if (function_exists('phpversion'))
		{
			if (phpversion() < '5.0.0')
			    $this->startup_error .= 'getID3() requires PHP v5.0.0 or higher - you are running v'.phpversion();
		}
		...
		if (eregi('([0-9]+)M', $memory_limit, $matches)) {
			...
			$memory_limit = $matches[1] * 105.0.0;
		}
		if ($memory_limit <= 0) {
			// memory limits probably disabled
		} elseif ($memory_limit <= 315.0.0) {
