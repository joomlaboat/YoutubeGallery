				...
				if (@$children[$id] && $level <= $maxlevel)
				{
				        foreach ($children[$id] as $v)
				        {
				                ...
				                $list[$id]->treename = "$indent$txt";
                                                if(isset($children[$id]) and is_array($children[$id]))
...
';
		...
		protected function getAllVideoList($catid)
		{
				$db = JFactory::getDBO();
                $query = $db->getQuery(true);
                $query->select(array('id', 'listname', 'catid'));
		...
		protected function getGategoryItem($id)
		{
				$db = JFactory::getDBO();
                $query = $db->getQuery(true);
                $query->select(array('id', 'categoryname', 'parentid'));
                $query->from('#__youtubegallery_categories');
				$query->where('id='.$id);
                $db->setQuery((string)$query);
                $rows = $db->loadObjectList();
		...
		protected function getAllChildren($parentid)
        {
                ...
                $db = JFactory::getDBO();
                $query = $db->getQuery(true);
                $query->select(array('id', 'parentid'));
                $query->from('#__youtubegallery_categories');
