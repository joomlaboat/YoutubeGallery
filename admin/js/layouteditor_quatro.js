...
function showModalTagForm(tagstartchar,tagendchar,tag,top,left,line,positions,isnew,param_count)
{
    ...
    jQuery(function($)
    {
		...
		const popoverElement = $(obj).find(".hasPopover");
		//popoverElement.popover({"html": true,"trigger": "hover focus","layouteditor_modal_content_box": "body"});
    });
...
function render_current_TagSets(index,layoutname,tagsets_)
{
    ...
    for(let i=0;i<tagsets_.length;i++)
    {
        ...
        if (proversion || typeof(a.proversion) === "undefined" || a.proversion==="0")
        {
			...
			//result_li = result_li + '<button aria-controls="layouteditor_tags'+index+'_'+i+'" role="tab" type="button" ' + (i==0 ? ' aria-expanded="true"' : '')+ '>'+a.label+(i)+'</button>';
...
//			result_div+='<button aria-expanded="false" aria-controls="layouteditor_tags'+index+'_'+i+'" type="button" hidden=""><span class="accordion-title">'+a.label+'<span class="accordion-icon"></span></span></button>';
	...
	let tags_box=document.getElementById("tags_"+layoutname);
	if(tags_box)
