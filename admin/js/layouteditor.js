...
function updateTagsParameters() {
    for (let i = 0; i < text_areas.length; i++) {
        const layoutname = text_areas[i][0];
        const index = text_areas[i][1];

        const tagsets_ = findTagSets(layoutname);
        if (tagsets_.length > 0)
            render_current_TagSets(index, layoutname, tagsets_);
        else {
            const tags_box_obj = document.getElementById("tags_" + layoutname);
